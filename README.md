...
* On the client, verify DNS configuration using  
> ipconfig /all.  
...
* Verify user's security token using  
> whoami /groups.

### Signup, Set up, and Explore ServiceNow PDI Incident Management Tool  
### Signup and Setup for ServiceNow Developer PDI  
* Click [Here](https://signon.servicenow.com/x_snc_sso_auth.do?pageId=login) to visit ServiceNow Developer login website.  
* Click New User, sign up, and verify the account.  
* After logging in, on the top right of your screen, click Create Instance. Select the latest version/build.  
* Click your newly built Instance URL.  
* Top left of your screen in the new page, click All. search Incident and click it.  
### Understanding the fundamentals of ServiceNow Incident Tool
* Quick rundown of the Incident Dashboard.  
<img width="1915" height="910" alt="service1" src="https://github.com/user-attachments/assets/62cd1e22-d796-4dbb-a81d-ef9709ec6b9b" />  

* Number = Incident case number.  
* Priority = Sort Incidents to your preference. Default set 1 for priority to 5 for planning.  
* State = Status of the incident.  
* Assignment group and Assigned to = Determine which group and person is set for each incident. Can set up automatic assignment per company policy.  
* New = Create a new incident.  
* Quick rundown of an Incident.
<img width="1908" height="908" alt="service2" src="https://github.com/user-attachments/assets/5275886f-023e-43e6-a558-d57edfc9f05b" />

* Number = Incident case number.  
* Caller = Required field. Fill in the name of the Caller. Can search for or create new caller ID by clicking the magnifying glass to access Caller ID database.   
* Category, Subcategory = Classifies the issue.  
* Service, Service offering = The overarching business service affected by the incidient. This category depends on company policy (SLA for service offerings).  
* Configuration Item = Specific hardware, software, or asset that is failing or needs attention.  
* Channel = How the issue was reported.  
* State = Current stage of the ticket lifecycle.  
* Impact = Measures the effect of the issue on the business.  
* Urgency = Measures how quickly the business needs the issue resolved.  
* Priority = A calucation automatically generated by the system based on Impact and Urgency.  
* Assignment Group, Assigned to = assign specific group and person to resolve the issue.  
* Related Search Results = Click to search knowledge database of similar incidients to help theorize solutions.  
* Notes = Internal notes used by IT to document troubleshooting steps, updates, and communication between the team.  
* Resolution Information = Documenting the resolution.  
